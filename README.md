...
- `bind` reads multi-line commands (blank line or Ctrl-D to finish), then records the key combo in your terminal. Esc cancels, Cmd can't be recorded. Combos need a modifier; bare F-keys are the exception. Recording assumes a US keyboard layout.
...
- The menu bar shows every binding (click to run) plus Reload and Quit; config changes are picked up within a couple of seconds.
