...
enum HotkeyCapture {
    ...
    static func record(existing: [Binding]) -> (keyCode: UInt32, modifiers: UInt32)? {
        let panel = NSPanel(
        ...
        let monitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
            guard event.window === panel, !confirming else { return event }
            ...
            if modifiers == 0 && !Config.bareKeys.contains(UInt32(event.keyCode)) {
            ...
            if status == eventHotKeyExistsErr {
                ...
                label.stringValue = "Couldn't register that combination (\(status))"
                return nil
            }
